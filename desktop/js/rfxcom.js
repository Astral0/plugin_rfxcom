 ...
 */
 ...
 $(document).ready(function() {
    $('.eqLogicAttr[data-l1key=configuration][data-l2key=device]').on('change', function () {
      if($('.li_eqLogic.active').attr('data-eqlogic_id') != ''){
        ...
        $('#img_device').attr("src", 'plugins/rfxcom/core/config/devices/'+$(this).value()+'.jpg');
    }else{
        $('#img_device').attr("src",'plugins/rfxcom/plugin_info/rfxcom_icon.png');
    }  
});
 ...
 $('#bt_changeBatteryTo').on('click',function(){
    $('#span_eqLogicFromId').value($('.eqLogicAttr[data-l1key=id]').value());
    $('#span_eqLogicFromName').value($('.eqLogicAttr[data-l1key=name]').value());
    $('#sel_eqLogicToId option').show();
    $('#sel_eqLogicToId option[value='+$('.eqLogicAttr[data-l1key=id]').value()+']').hide();
    $("#md_changeBattery").dialog('open');
});
 
 $('#bt_cancelChangeBattery').on('click',function(){
    $("#md_changeBattery").dialog('close');
});

 $('#bt_valideChangeBattery').on('click',function(){
    $("#md_changeBattery").dialog('close');
    $.ajax({
        type: "POST",
        url: "plugins/rfxcom/core/ajax/rfxcom.ajax.php", 
        data: {
            action: "changeBattery",
            from: $('#span_eqLogicFromId').value(),
            to:  $('#sel_eqLogicToId').value(),
        },
        dataType: 'json',
        ...
        success: function (data) {
            ...
            url += 'id=' +  $('#sel_eqLogicToId').value();
            window.location.href = url;
        }
    });
});

 $('#bt_healthrfx').on('click', function () {
    $('#md_modal').dialog({title: "{{Santé Rfxcom}}"});
    $('#md_modal').load('index.php?v=d&plugin=rfxcom&modal=health').dialog('open');
});
 ...
 function getModelList(_conf,_id) {
    $.ajax({
        ...
        success: function (data) {
			...
			if($icon != '' && $icon != null){
				$('#img_device').attr("src", 'plugins/rfxcom/core/config/devices/'+$icon+'.jpg');
			}
        }
    });
}
...
$('body').off('rfxcom::includeDevice').on('rfxcom::includeDevice', function (_event,_options) {
    if (modifyWithoutSave) {
        ...
        if (_options == '') {
            ...
            window.location.href = 'index.php?v=d&p=rfxcom&m=rfxcom&id=' + _options;
        }
    }
});
