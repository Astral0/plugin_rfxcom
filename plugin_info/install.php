...
function rfxcom_install() {
	if (config::byKey('api::rfxcom::mode') == '') {
		config::save('api::rfxcom::mode', 'localhost');
	}
}

function rfxcom_update() {
	if (config::byKey('api::rfxcom::mode') == '') {
		config::save('api::rfxcom::mode', 'localhost');
	}
	foreach (eqLogic::byType('rfxcom') as $eqLogic) {
		...
		$device = rfxcom::devicesParameters($packettype);
